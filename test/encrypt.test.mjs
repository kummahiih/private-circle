...
describe('encryptPage (integration)', () => {
  ...
  it('encrypts a content directory into files{} map (multifile v2)', () => {
    const contentDir = path.join(root, 'content-tree');
    fs.mkdirSync(contentDir);
    fs.writeFileSync(
      path.join(contentDir, 'index.html'),
      '<!DOCTYPE html><html><head><link rel="stylesheet" href="app.css"></head><body><h1>MARKER-DIR</h1><script src="app.js"></script></body></html>'
    );
    fs.writeFileSync(path.join(contentDir, 'app.js'), 'console.log("MARKER-DIR-JS");');
    fs.writeFileSync(path.join(contentDir, 'app.css'), 'body { color: red; /* MARKER-DIR-CSS */ }');

    const outMulti = path.join(root, 'dist-multi');
    const result = encryptPage({
      pageId: 'test-page',
      content: contentDir,
      hashes: hashesDir,
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(outMulti, 'gate-config.json'), 'utf8'));
    assert.equal(cfg.v, 2);
    assert.ok(cfg.files);
    assert.ok(cfg.files['index.html']);
    assert.ok(cfg.files['app.js']);
    ...
    assert.ok(!index.includes('MARKER-DIR-JS'));
    assert.ok(!fs.existsSync(path.join(outMulti, 'content')));
    assert.ok(!fs.existsSync(path.join(outMulti, 'app.js')));
    assert.ok(!fs.existsSync(path.join(outMulti, 'app.css')));
    ...
    assert.ok(cspMatch);
    assert.equal(cspMatch[1], GATE_CSP);
    assert.ok(cspMatch[1].includes('blob:'));
  });

  it('single-file mode still works (backward compat, no files map required)', () => {
    const outSingle = path.join(root, 'dist-single');
    const result = encryptPage({
      pageId: 'test-page',
      content: contentFile,
      hashes: hashesDir,
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(outSingle, 'gate-config.json'), 'utf8'));
    assert.equal(cfg.v, 1);
    assert.ok(cfg.cipher && cfg.iv);
    assert.equal(cfg.files, undefined);
  });

  it('hygiene fails if plaintext js would appear in dist (marker scan)', () => {
    const contentDir = path.join(root, 'content-leak');
    fs.mkdirSync(contentDir);
