...
- AES-GCM authentication failures while probing derived keys: WebCrypto surfaces both wrong-`K` and a bad auth tag as `OperationError`. Gate still tries the next entry; unexpected exception names are logged.
...
- **AES-GCM IV reuse under shared `K`:** Multifile builds use one content key `K` with a unique 12-byte IV per file. Encrypt tracks used IVs and fails closed on collision. *Residual:* astronomical chance of exhausting retries (build abort, no reuse).
- **Weak PBKDF2 parameters:** Encrypt rejects enroll JSON with iterations &lt; 310000. *Residual:* operator must still use a strong password or PRF.
...
- Missing or wrong `Content-Type` on static assets (e.g. `gate.js` as `text/plain`) can interact badly with CSP and script execution. Required headers are documented in the operator runbook.
...
- [x] **Reject AES-GCM IV collisions** when encrypting multiple files under the same `K` (`src/encrypt-page.mjs`). Track used IVs and fail the build on reuse; do not rely on 12-byte randomness alone.
- [x] **Fail closed on weak PBKDF2 iterations** in `src/load-hashes.mjs`: throw if `PBKDF2-SHA256` iterations &lt; 310000 instead of `console.warn`.
- [x] **Distinguish AES-GCM auth-tag failures** in `assets/gate.js` key-probe `catch` (defense in depth). Still try the next entry; log unexpected (non-`OperationError`) probe failures. WebCrypto cannot separate wrong-`K` from a bad tag.
...
- [x] **Document required HTTP `Content-Type` headers** in the operator runbook (`gate.js` → `application/javascript`, `gate.css` → `text/css`, `gate-config.json` → `application/json`, `index.html` → `text/html; charset=utf-8`, plus CSP on all responses).
